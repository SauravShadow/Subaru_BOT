...
async def _cf_add_tunnel_route(subdomain: str, port: int) -> tuple[bool, str]:
    """Add a public hostname to the Cloudflare Tunnel via API."""
    if not CF_ACCOUNT_ID or not CF_API_TOKEN:
        return False, "CF_ACCOUNT_ID or CF_API_TOKEN not set in .env"
    ...
    cfg_url = f"https://api.cloudflare.com/client/v4/accounts/{CF_ACCOUNT_ID}/cfd_tunnel/{CF_TUNNEL_ID}/configurations"

    try:
        resp = await client.get(cfg_url, headers=headers, timeout=15)
        resp.raise_for_status()
        data = resp.json()
    except Exception as e:
        return False, f"Failed to fetch tunnel config: {e}"
...
async def _cf_remove_tunnel_route(subdomain: str) -> tuple[bool, str]:
    """Remove a public hostname from the Cloudflare Tunnel config."""
    if not CF_ACCOUNT_ID or not CF_API_TOKEN:
        return False, "CF credentials not set"
    ...
    try:
        resp = await client.get(cfg_url, headers=headers, timeout=15)
        resp.raise_for_status()
        ingress = resp.json().get("result", {}).get("config", {}).get("ingress", [])
    except Exception as e:
        return False, f"Failed to fetch tunnel config: {e}"

    updated = [r for r in ingress if r.get("hostname") != hostname]
    if len(updated) == len(ingress):
        return True, f"{hostname} not found in tunnel config — nothing to remove"
...
def _nginx_add_vhost(subdomain: str, port: int) -> tuple[bool, str]:
    """Generate and enable a simple nginx server block for subdomain → port."""
    hostname = f"{subdomain}.{CF_DOMAIN}"
    config = f"""# Auto-generated by operations_sidecar for {hostname}
server {{
    listen 80;
...
"""
    site_path = f"/etc/nginx/sites-available/{subdomain}"
    ...
    try:
        ...
        if result.returncode != 0:
            return False, f"tee failed: {result.stderr.decode()}"
        subprocess.run(["sudo", "ln", "-sf", site_path, link_path], check=True, timeout=10)
        test = subprocess.run(["sudo", "nginx", "-t"], capture_output=True, timeout=10)
        if test.returncode != 0:
            return False, f"nginx -t failed: {test.stderr.decode()}"
        subprocess.run(["sudo", "nginx", "-s", "reload"], check=True, timeout=10)
        return True, f"nginx: added {hostname} → port {port} and reloaded"
    except Exception as e:
        return False, f"nginx vhost failed: {e}"


def _nginx_remove_vhost(subdomain: str) -> tuple[bool, str]:
    """Remove and disable an nginx server block."""
    site_path = f"/etc/nginx/sites-available/{subdomain}"
    link_path = f"/etc/nginx/sites-enabled/{subdomain}"
    try:
        subprocess.run(["sudo", "rm", "-f", link_path, site_path], check=True, timeout=10)
        subprocess.run(["sudo", "nginx", "-s", "reload"], check=True, timeout=10)
        return True, f"nginx: removed {subdomain} vhost and reloaded"
    except Exception as e:
        return False, f"nginx remove failed: {e}"
...
async def api_start_service(body: dict):
    ...
    _service_registry[name] = {"cwd": cwd, "cmd": cmd, "port": port, "subdomain": subdomain}
    _save_registry(_service_registry)
    if port:
        _register_port(port, service=name, subdomain=subdomain, note=f"cwd:{cwd}")

    await asyncio.sleep(2)

    # Auto-wire Cloudflare Tunnel route if subdomain provided
    cf_msg = ""
    if subdomain and port:
        cf_ok, cf_msg = await _cf_add_tunnel_route(subdomain, port)
        await broadcast({"type": "log", "text": f"[cloudflare] {cf_msg}"})
    ...
    return {
        ...
        "cf":        cf_msg or None,
        "message":   f"Service '{name}' started{f' at {subdomain}.{CF_DOMAIN}' if subdomain else ''}. Logs: /tmp/{name}.log",
    }
...
async def api_stop_service(body: dict):
    ...
    # Remove from service registry, port registry, and Cloudflare Tunnel
    cf_msg = None
    if name in _service_registry:
        info = _service_registry.pop(name)
        _save_registry(_service_registry)
        if info.get("port"):
            _unregister_port(info["port"], subdomain=info.get("subdomain", ""))
        if info.get("subdomain"):
            _, cf_msg = await _cf_remove_tunnel_route(info["subdomain"])
            logger.info("[cloudflare] %s", cf_msg)
    ...
    out, _ = await proc.communicate()
    return {"ok": True, "result": out.decode().strip(), "cf": cf_msg}
...
async def api_register_subdomain(body: dict):
    """Register a subdomain→port mapping and auto-wire the Cloudflare Tunnel route."""
    port = body.get("port")
    subdomain = body.get("subdomain", "").strip()
    service = body.get("service", subdomain)
    note = body.get("note", "")
    skip_cf = body.get("skip_cf", False)
    nginx = body.get("nginx", False)
    if not port or not subdomain:
        return {"ok": False, "error": "port and subdomain required"}

    port = int(port)
    _register_port(port, service=service, subdomain=subdomain, note=note)

    messages = [f"Registered {subdomain}.{CF_DOMAIN} → port {port}"]

    if not skip_cf:
        cf_ok, cf_msg = await _cf_add_tunnel_route(subdomain, port)
        messages.append(cf_msg)
        logger.info("[cloudflare] %s", cf_msg)

    if nginx:
        ng_ok, ng_msg = _nginx_add_vhost(subdomain, port)
        messages.append(ng_msg)
        logger.info("[nginx] %s", ng_msg)
