...
def test_silence_should_fire():
    sec = router._SILENCE_MS / 1000.0
    class S:
        last_interim_text = "book a table"
        last_interim_at = 1000.0
        responded_text = ""
    s = S()
    assert router._silence_should_fire(s, now=1000.0 + sec + 0.05) is True   # past threshold
    assert router._silence_should_fire(s, now=1000.0 + sec - 0.10) is False  # still within
    s.responded_text = "book a table"                                        # already answered
    assert router._silence_should_fire(s, now=1000.0 + sec + 1.0) is False
    s.responded_text = ""
    s.last_interim_text = ""                                                 # nothing pending
    assert router._silence_should_fire(s, now=1000.0 + sec + 1.0) is False
